...
class TestSnapshotBuyPutNoCosts:
    ...
    def test_final_capital(self):
        ...
        assert abs(final - 982110.0) < 0.01, f"Regression: final_capital={final}"
    ...
    def test_total_return(self):
        ...
        assert abs(ret - (-0.01789)) < 1e-4, f"Regression: total_return={ret}"
    ...
    def test_max_drawdown(self):
        ...
        max_dd = dd.max()
        assert abs(max_dd - 0.039055) < 1e-4, f"Regression: max_drawdown={max_dd}"
...
class TestSnapshotBuyPutWithCommission:
    ...
    def test_final_capital(self):
        ...
        assert abs(final - 980109.35) < 0.01, f"Regression: final_capital={final}"
...
class TestSnapshotSellPut:
    ...
    def test_final_capital(self):
        final = self.engine.balance["total capital"].iloc[-1]
        assert abs(final - 172880.0) < 1.0, f"Regression: final_capital={final}"
